...
async function evaluateAnswer(questionText, candidateAnswer, expectedConcepts = [], topic = '') {
  try {
    const prompt = `You are an expert interviewer evaluating a candidate's response to an interview question.

Question: "${questionText}"
Topic: "${topic}"
Expected Key Concepts/Keywords: ${JSON.stringify(expectedConcepts)}
...
Output ONLY a valid JSON object matching this schema:
{
  ...
  "isRelevant": boolean
    ...
    const evaluation = await callStructured(prompt, evaluationSchema, {
      generationConfig: {
        temperature: 0.1, // Low temperature for consistent grading
      }
    });

    logger.info('[EvaluatorService] Successfully evaluated answer', {
